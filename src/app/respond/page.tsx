...
import { Send, User, Loader2, Check, X, Calendar, AlertCircle } from "lucide-react";
    ...
    if (!candidateId || isLegacyLink) return (
        <div className="min-h-screen flex items-center justify-center bg-surface p-5">
            <div className="max-w-sm w-full bg-white rounded-[12px] border border-border p-8 text-center" style={{ boxShadow: "var(--shadow-soft)" }}>
                ...
                >
                    Invalid or Legacy Link
                </h1>
                <p className="text-muted text-[12.5px] leading-relaxed">Please use the latest invitation link from your email.</p>
            </div>
        </div>
    );
            ...
            <main className="max-w-md w-full bg-white rounded-[12px] border border-border overflow-hidden" style={{ boxShadow: "var(--shadow-premium)" }}>
                ...
                <div className="p-6 space-y-5">
                    <div className="p-3.5 rounded-md bg-surface border border-border flex items-center gap-3">
                        <div className="w-10 h-10 rounded-md bg-heading flex items-center justify-center shrink-0">
                            <User className="w-4 h-4 text-white" strokeWidth={1.5} />
                        </div>
                        <div className="min-w-0">
                            <p className="text-[13px] font-semibold text-heading truncate">{interviewerName || "Anonymous Representative"}</p>
                            <p className="text-[10px] text-muted font-medium uppercase tracking-[0.12em] mt-0.5 truncate">{email || 'system@cbt.com'}</p>
                        </div>
                    </div>
