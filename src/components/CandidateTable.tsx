...
const statusColors: Record<CandidateStatus, string> = {
    ...
    "Invite Sent": "bg-blue-50 text-blue-700 border-blue-200 font-bold",
                                        ...
                                        <tr className="bg-gradient-to-r from-primary/[0.03] to-transparent">
                                            <td colSpan={8} className="px-4 py-3">
                                                <div className="flex flex-wrap items-center justify-end gap-3 fade-in slide-in-from-top-1 animate-in duration-200">
                                                    ...
                                                    {candidate.status === 'Recommended' && (isMaster || isHR) && (
                                                        <button
                                                            onClick={async () => {
                                                                if (!window.confirm(
                                                                ...
                                                                if (result.success) {
                                                                    setCandidates(prev =>
                                                                        prev.map(c => c.id === candidate.id
                                                                            ? { ...c, status: 'Selected' as CandidateStatus, last_action_by: result.last_action_by }
                                                                            : c
                                                                        )
                                                                    );
                                                                    alert(`Selection email sent to ${candidate.name} successfully!`);
                                                                } else {
                                                                    alert('Failed to send selection email: ' + result.error);
                                                                }
                                                            }}
                                                            disabled={sendingSelectionId === candidate.id}
                                                            className="flex items-center gap-1.5 px-4 py-2 bg-emerald-700 border border-emerald-800 text-white text-[11px] font-bold rounded-sm shadow-soft hover:bg-emerald-800 transition-colors disabled:opacity-60"
