...
import { Clock, CheckCircle, XCircle, MessageSquare, X, Eye, Calendar, User, FileText, Star, Activity, Send, Shield, ChevronRight, Search, ArrowDown, ArrowUp, MailWarning, Pencil } from "lucide-react";
import { cn, formatSlotDate, formatSlotTime } from "@/lib/utils";
import { withLoading } from "@/lib/loading";
import { UserRole, AssignableInterviewer, requestL2Interview, submitFinalInterviewFeedback, lockInterviewMeeting, generateAndLockInterview, getInterviewerAvailability, resendL2Invitation, rejectForLowAssessmentScore, assignInterviewer } from "@/app/actions";
...
interface Interview {
    ...
    // Assigned interviewer (FK to public.users). Distinct from the
    // l1/l2_interviewer_name fields, which record who actually submitted
    // each feedback round.
...
interface InterviewListProps {
    ...
    userRoles: UserRole[];
    interviewers: AssignableInterviewer[];
}
...
export default function InterviewList({ initialInterviews, userRoles, interviewers }: InterviewListProps) {
    ...
    // ── Interviewer column ────────────────────────────────────────────────
    // Recruitment team (HR) and Master users can change the assigned
    // interviewer inline; everyone else sees it read-only.
    const canEditInterviewer = userRoles.includes('HR') || userRoles.includes('Master');
    const [editingInterviewerFor, setEditingInterviewerFor] = useState<string | null>(null);
    const interviewerNameById = useMemo(
        () => new Map(interviewers.map(u => [u.id, u.full_name])),
        [interviewers]
    );
    const selectableInterviewers = useMemo(
        () => interviewers.filter(u => u.canInterview),
        [interviewers]
    );
    // Name shown in the column: the explicit assignment wins; otherwise fall
    // back to whoever submitted feedback, so historical rows aren't blank.
    const displayedInterviewerName = (i: Interview): string | null => {
        if (i.interviewer_id) return interviewerNameById.get(i.interviewer_id) ?? 'Unknown user';
        return i.l2_interviewer_name || i.l1_interviewer_name || null;
    };

    const handleAssignInterviewer = async (interviewId: string, interviewerId: string | null) => {
        setIsSubmitting(true);
        try {
            const result = await withLoading(() => assignInterviewer(interviewId, interviewerId));
            if (result.error) throw new Error(result.error);
            setInterviews(prev => prev.map(i =>
                i.id === interviewId ? { ...i, interviewer_id: interviewerId } : i
            ));
            setEditingInterviewerFor(null);
            router.refresh();
        } catch (err: any) {
            alert(err.message);
        } finally {
    ...
    const visibleInterviews = useMemo(() => {
        ...
        const matches = interviews.filter(i => {
            ...
            // Search by candidate or interviewer name (case-insensitive)
            if (search.trim()) {
                const needle = search.trim().toLowerCase();
                const name = i.candidates?.name?.toLowerCase() ?? '';
                const interviewer = displayedInterviewerName(i)?.toLowerCase() ?? '';
                if (!name.includes(needle) && !interviewer.includes(needle)) return false;
            }
        ...
        return sorted;
    }, [interviews, statusFilter, timeFilter, search, sortDir, interviewerNameById]);
                ...
                <table className="w-full text-left border-collapse table-fixed" style={{ minWidth: '900px' }}>
                    <thead>
                        <tr className="bg-surface border-b border-border">
                            <th className="px-6 py-3 text-[9px] font-black text-muted uppercase tracking-[0.2em] w-[14%]">Scheduled</th>
                            <th className="px-6 py-3 text-[9px] font-black text-muted uppercase tracking-[0.2em] w-[20%]">Candidate</th>
                            <th className="px-6 py-3 text-[9px] font-black text-muted uppercase tracking-[0.2em] w-[16%]">Interviewer</th>
                            <th className="px-6 py-3 text-[9px] font-black text-muted uppercase tracking-[0.2em] w-[18%]">Interview Status</th>
                            <th className="px-6 py-3 text-[9px] font-black text-muted uppercase tracking-[0.2em] w-[20%]">Meeting</th>
                            <th className="px-6 py-3 text-[9px] font-black text-muted uppercase tracking-[0.2em] text-right w-[12%]">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-[var(--border)]/50">
                        {visibleInterviews.length === 0 && (
                            <tr>
                                <td colSpan={6} className="px-6 py-12 text-center text-muted text-[12px] font-medium">
                                    {hasActiveFilters
                        ...
                        {visibleInterviews.map((interview) => {
                            ...
                            return (
                                <tr key={interview.id} className="hover:bg-[var(--surface)] transition-colors group">
                                    ...
                                    <td className="px-6 py-5 align-top">
                                        {editingInterviewerFor === interview.id ? (
                                            <div className="flex flex-col gap-1.5">
                                                <select
                                                    autoFocus
                                                    defaultValue={interview.interviewer_id ?? ''}
                                                    disabled={isSubmitting}
                                                    onChange={(e) => handleAssignInterviewer(interview.id, e.target.value || null)}
                                                    className="text-[12px] font-semibold text-heading bg-white border border-primary rounded-sm px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-primary/20 disabled:opacity-50"
                                                >
                                                    <option value="">— Unassigned —</option>
                                                    {selectableInterviewers.map(u => (
                                                        <option key={u.id} value={u.id}>{u.full_name}</option>
                                                    ))}
                                                </select>
                                                <button
                                                    type="button"
                                                    onClick={() => setEditingInterviewerFor(null)}
                                                    disabled={isSubmitting}
                                                    className="text-[9px] font-bold text-muted uppercase tracking-widest hover:text-heading w-fit"
                                                >
                                                    Cancel
                                                </button>
                                            </div>
                                            ...
                                            <div className="flex items-start gap-2 min-w-0">
                                                <div className="flex flex-col min-w-0">
                                                    {displayedInterviewerName(interview) ? (
                                                        <span className="text-[13px] font-bold text-heading truncate flex items-center gap-1.5">
                                                            <User className="w-3 h-3 text-muted shrink-0" />
                                                            {displayedInterviewerName(interview)}
                                                        </span>
                                                    ) : (
                                                        <span className="text-[11px] font-medium text-muted italic">Unassigned</span>
                                                    )}
                                                    {!interview.interviewer_id && displayedInterviewerName(interview) && (
                                                        <span className="text-[9px] text-muted uppercase tracking-widest mt-0.5">From feedback</span>
                                                    )}
                                                </div>
                                                {canEditInterviewer && (
                                                    <button
                                                        type="button"
                                                        onClick={() => setEditingInterviewerFor(interview.id)}
                                                        disabled={isSubmitting}
                                                        title="Change interviewer"
                                                        className="p-1 rounded-sm text-muted hover:text-primary hover:bg-primary/5 transition-colors opacity-60 group-hover:opacity-100 shrink-0"
                                                    >
